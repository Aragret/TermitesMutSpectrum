...
################### 
Taxa = read.csv("../../Body/2Derived/CockroachesFamilies.csv") 
Final = read.table("../../Body/3Results/Cockroaches.Normalization.NeutralATGC.txt",
                   header = TRUE, sep = '\t')
...
MUT = merge(MUT, Taxa[, c('Species', 'Family')], all.x = TRUE)
...
write.table(PieChartTable,"../../Body/3Results/Cockroaches.Polymorphisms.BetweenFamiliesWithoutNormalization.PieChartTable.txt")
Equil = data.frame(Equil); names(Equil) = c('i','Classes','G','T','C','A')

##################

PieChartTable = read.table("../../Body/3Results/Cockroaches.Polymorphisms.BetweenFamiliesWithoutNormalization.PieChartTable.txt")
...
write.table(PieChartTable,"../../Body/3Results/Cockroaches.Polymorphisms.Normalization.Normalized12Fractions.txt", quote = FALSE, row.names = FALSE)
...
NormFrac = read.table("/home/glebo/Documents/lab/TermitesMutSpectrum/Body/3Results/Cockroaches.Polymorphisms.Normalization.Normalized12Fractions.txt", header = TRUE)


pdf("/home/glebo/Documents/lab/TermitesMutSpectrum/Body/4Figures/Cockroaches.Polymorphisms.Normalization.R.03.Bars.pdf", width = 15, height = 10)

b<-ggplot(NormFrac, aes(x="Subs", y="Normalised2Number"), xlab="Substitution types", ylab="Normalised frequencies")
b

NormFrac$Subs = as.factor(NormFrac$Subs) 
...
b = ggbarplot(NormFrac[NormFrac$Family == 'Cryptocercidae',], 'Subs', 'Normalised2Number', 
              xlab="Substitution types", ylab = 'Normalized frequencies', title = 'Cryptocercidae',
              fill = 'Subs', color = 'Subs', palette = c("#bdbdbd", "#7fcdbb", "#bdbdbd", "#bdbdbd", "#bdbdbd", "#feb24c", "#f03b20", "#bdbdbd", "#bdbdbd", "#bdbdbd", "#2c7fb8", "#bdbdbd"), combine = TRUE) +
  scale_x_discrete(labels = sort(unique(NormFrac$Subs), decreasing = TRUE))
...
normMutCock[6, "T_C_norm_fr"] = 0 #wtf do I do with that?
...
barplot(normMutCock$G, names.arg = normMutCock$Species, )
...
# t = 8.3365, df = 53.696, p-value = 2.957e-11
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
# 0.1800588 0.2941073
# sample estimates:
#   mean of x mean of y 
# 0.32780823 0.09072514 
